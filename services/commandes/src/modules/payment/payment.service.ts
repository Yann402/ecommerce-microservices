...
export class PaymentService {
  ...
  async pay(montant: string): Promise<{ success: boolean; transactionId?: string }> {
    let success: boolean;
    switch (this.mode) {
      case 'always_fail': success = false; break;
      case 'random': success = Math.random() > 0.3; break;
      default: success = true;
    }
    this.logger.log(`Paiement mock (${this.mode}) de ${montant} -> ${success ? 'accepté' : 'refusé'}`);
